...
export class IdentitiesController {
  ...
  @ApiOperation({
    ...
    description: "Crée le profil personnel de l'utilisateur connecté",
  })
  @ApiCreatedResponse({ description: 'Profil créé' })
  @UseGuards(JwtAuthGuard)
  @Throttle({ default: { ttl: 60_000, limit: 5 } })
  @Post('profile')
  async createProfile(@CurrentUser() user: AuthenticatedUser, @Body() dto: CreateProfileDto) {
  ...
  @ApiOperation({
    ...
    description: 'Télécharge une photo de profil (jpeg, png, webp, max 5 Mo)',
  })
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Générer QR code',
    description: 'Génère ou retourne le QR code personnel. Ajouter ?force=true pour forcer la régénération',
  })
  @ApiCreatedResponse({ description: 'QR code généré' })
  @UseGuards(JwtAuthGuard)
  @Throttle({ default: { ttl: 60_000, limit: 5 } })
  @Post('profile/qrcode')
  async generateQrCode(@CurrentUser() user: AuthenticatedUser, @Query('force') force?: string) {
    return this.identitiesService.generateQrCode(user.userId, force === 'true');
  }

  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Vérifier profil',
    description:
      "Marque le profil comme vérifié (réservé aux administrateurs, nécessite une pièce d'identité uploadée)",
  })
  ...
  @Throttle({ default: { ttl: 60_000, limit: 5 } })
  @Post('profile/verify')
  async verifyProfile(@CurrentUser() user: AuthenticatedUser, @Body('userId') targetUserId?: string) {
    const userId = targetUserId ?? user.userId;
    return this.identitiesService.verifyProfile(userId);
  }

  @ApiBearerAuth()
  @ApiOperation({
    summary: "Upload pièce d'identité",
    description: "Télécharge une photo de la pièce d'identité (jpeg, png, webp, pdf, max 10 Mo)",
  })
  ...
  async uploadIdentityDocument(
    @CurrentUser() user: AuthenticatedUser,
    @UploadedFile(identityDocumentFilePipe)
    file: { buffer: Buffer; mimetype: string; originalname: string; size: number },
  ) {
  ...
  @ApiOperation({
    ...
    description: 'Télécharge une signature numérique (jpeg, png, webp, max 2 Mo)',
  })
  ...
  async uploadDigitalSignature(
    @CurrentUser() user: AuthenticatedUser,
    @UploadedFile(signatureFilePipe)
    file: { buffer: Buffer; mimetype: string; originalname: string; size: number },
  ) {
  ...
  @ApiOperation({
    ...
    description: "Retourne les informations publiques d'un profil par son ID utilisateur",
  })
